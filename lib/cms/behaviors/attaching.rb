    ...
    module Attaching
      ...
      module InstanceMethods
        ...
        def attachment_named(name)
          attachments.select { |item| item.attachment_name.to_sym == name && (item.published && !item.deleted && !item.archived) && item.data_content_type.present? }.uniq{ |x| x.created_at }.last
        end
